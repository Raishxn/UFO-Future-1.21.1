...
Alcance padrão por origem acompanha `wireless.range` (128 por padrão). A UI permite ajustar uma origem individualmente (+1 / Shift -1), com esse valor como teto global. Origens antigas que ainda guardavam o padrão anterior de 32 blocos passam a acompanhar a configuração; ajustes individuais novos, inclusive 32, são marcados e persistidos. `wireless.range = 0` remove o limite de distância. Reduzir o alcance suspende transferências fora dele sem apagar vínculos. Configurações wireless antigas em ufo-server.toml não são mais lidas; fonte autoritativa é config/ufo/wireless.toml no servidor. Os snapshots dos jobs existentes são mantidos.
